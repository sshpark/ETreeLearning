...
public class Matrix implements Serializable {
    ...
    public Matrix broadcastTo(int rows, int cols) {
        if (numberOfColumns != 1 && numberOfRows != 1) {
            throw new RuntimeException("One of the dimensions should be 1");
        }
        Matrix res = new Matrix(rows, cols);
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < cols; j++) {
                if (numberOfRows == 1)
                    res.matrix[i][j] = matrix[0][j];
                else
                    res.matrix[i][j] = matrix[i][0];
            }
        }
        return res;
    }
